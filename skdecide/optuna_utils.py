...
def generic_optuna_experiment_monoproblem(
    ...
    aggreg_outcome_rewards: Optional[
        Callable[[D.T_agent[Value[D.T_value]]], float]
    ] = None,
    ...
    optuna_tuning_direction: direction of optuna tuning ("maximize" or "minimize")
    ...
    if pruner is None:
        pruner = MedianPruner()
    if aggreg_outcome_rewards is None:
    ...
    def objective(trial: Trial):
        ...
        try:
            with solver_class(domain_factory=domain_factory, **kwargs) as solver:
                solver.solve()
                ...
                if _is_rollout_deterministic:
                    num_episodes = 1
                    cumulated_value_multiplier = rollout_num_episodes
                else:
                    num_episodes = rollout_num_episodes
                    cumulated_value_multiplier = 1
            ...
            # cumulated cost/reward
            cumulated_value = cumulated_value_multiplier * sum(
                [
